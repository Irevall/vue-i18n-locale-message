        ...
        messages = blocks.reduce((messages, block) => {
            ...
            locales.reduce((messages, locale) => {
                if (block.messages[locale]) {
                    ...
                    Object.assign(target, deepmerge_1.default(target, localeBlockMessages));
                    return messages;
                }
                return messages;
            }, messages);
...
function splitLocaleMessages(path, messages, format) {
    ...
    const write = () => {
        locales.forEach(locale => {
            ...
            fs_1.default.writeFileSync(`${path}/${locale}.${format}`, utils_1.stringifyContent(messages[locale], format));
        });
    };
    try {
        write();
